...
// writeManifest writes a minimal melon.yml with the given deps to dir.
func writeManifest(t *testing.T, dir string, deps map[string]string) {
	...
	require.NoError(t, os.WriteFile(filepath.Join(dir, "melon.yml"), data, 0644))
}
...
func TestRunRemove_RemovesDependencyFromManifest(t *testing.T) {
	...
	// runRemove will call runInstall after saving melon.yml. runInstall will
	...
	m, err := manifest.Load(filepath.Join(dir, "melon.yml"))
	require.NoError(t, err)

	assert.NotContains(t, m.Dependencies, "alice/pdf-skill", "removed dep must not be in melon.yml")
	assert.Contains(t, m.Dependencies, "bob/base-utils", "remaining dep must still be in melon.yml")
}
...
func TestRunRemove_ErrorOnUnknownDep(t *testing.T) {
	...
	manifestBefore, err := os.ReadFile(filepath.Join(dir, "melon.yml"))
	require.NoError(t, err)
	...
	// melon.yml must be unchanged.
	manifestAfter, err := os.ReadFile(filepath.Join(dir, "melon.yml"))
	require.NoError(t, err)
	assert.Equal(t, string(manifestBefore), string(manifestAfter), "melon.yml must not be modified on error")
}
