...
The local package cache directory SHALL be named `.melon/` (previously `.mln/`). All commands that create, read, or reference the store directory SHALL use this name.
...
The project manifest file SHALL be named `melon.yml` (previously `mln.yaml`). All commands that read or write the manifest SHALL use this filename.
...
- **THEN** the success message references `melon.yml`, not `mln.yaml`

### Requirement: Lock file is named melon.lock
The reproducible lock file SHALL be named `melon.lock` (previously `mln.lock`). All commands that read or write the lock file SHALL use this filename.
