...
### Requirement: mln init writes CLI version to melon.yml
The `version` field written to `melon.yml` during `mln init` SHALL match the running CLI's version rather than a hardcoded value.

#### Scenario: Release binary sets version in melon.yml
- **WHEN** `mln init` is run with a release binary built at version `0.2.0`
- **THEN** the generated `melon.yml` contains `version: 0.2.0`

#### Scenario: Dev build sets version in melon.yml
- **WHEN** `mln init` is run with a binary built without ldflags version injection
- **THEN** the generated `melon.yml` contains `version: dev`
